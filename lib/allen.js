function allen(maxPremiumCapTable,silverRateTable){
	...
	d3.csv(maxPremiumCapTable,function(data){
		...
		eventDispatcher.on('householdInput',function(_size){
			...
			// userInput.agi <= userInput.agiHouseholdMax ? eventDispatcher.calc() : eventDispatcher.calcAlternate(userInput.agi);
		});
	...
	d3.csv(silverRateTable,function(data){
		...
		function premiumCredit(){
			...
			userInput.premiumCredit = d3.max([credit,0]);
			var temp = +Math.floor(userInput.premiumCredit);
			console.log(temp);
			d3.select("#premiumCredit").text(format(temp));
